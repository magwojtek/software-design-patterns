...
/**
 * Demonstrates the anti-pattern implementation of the Observer pattern
 * with tightly coupled observers that cannot be dynamically added/removed.
 */
function demonstrateAntiPattern(): void {
    ...
    logger.warn(
        'Problem: Cannot use different types of observers without modifying WeatherStation',
    );
    logger.warn('Problem: Hard to test due to tight coupling between subject and observers\n');
}

/**
 * Demonstrates the proper implementation of the Observer pattern
 * with loose coupling and dynamic observer management.
 */
function demonstrateProperPattern(): void {
    ...
    logger.log(
        `Smart home registered: ${smartHome instanceof SmartHomeSystem}`,
        LogColor.SMART_HOME,
    );
    ...
    class EmailAlertSystem implements Observer {
        update(temperature: number): void {
            if (temperature > 30 || temperature < 0) {
                ...
                logger.log(
                    `Email Alert: Normal temperature: ${temperature}°C`,
                    LogColor.EMAIL_ALERT,
                );
            }
        }
